...
<div x-data="{ open: $persist(true) }" wire:key="{{ $statePath }}" data-id="{{ $statePath }}" class="space-y-2"
    ...
    <div class="relative group">
        ...
        <div @class([
            ...
            <button x-init x-tooltip.raw.duration.0="{{ __('Add Child') }}" type="button"
                wire:click="addChild('{{ $statePath }}')" class="p-1" title="{{ __('Add Child') }}">
                @svg('heroicon-o-plus', 'w-3 h-3 text-gray-500 hover:text-gray-900')
            </button>

            <button x-init x-tooltip.raw.duration.0="{{ __('Remove') }}" type="button"
                wire:click="removeItem('{{ $statePath }}')" class="p-1" title="{{ __('Remove') }}">
