<x-guest-layout>
                ...
                <div
                    ...
                    <div class="lg:ms-10">
                        ...
                        <div class="mt-4">
                            ...
                            <a href="#!" data-type="youtube" data-id="{{ $settings->card2YoutubeURL }}"
    ...
    @if ($settings->enableSponsorSection)
        <section class="py-6 border-t border-b border-gray-100 dark:border-gray-700">
            <div class="container relative">
                <div class="grid md:grid-cols-6 grid-cols-2 justify-center gap-[30px]">
                    @foreach ($settings->sponsorImages as $item)
                        <div class="py-4 mx-auto">
                            <x-curator-glider class="h-6" :media="$item" />
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="container relative">
            ...
            <div class="grid lg:grid-cols-12 md:grid-cols-12 grid-cols-1 mt-8 gap-[30px] items-center">
                        ...
                        @foreach ($sports as $sport)
                            @if ($loop->odd)
                                <div class="duration-500 group md:text-end text-start">
                                    <x-curator-glider
                                        class="w-16 h-16 p-3 bg-white rounded-full shadow-md dark:shadow-gray-800"
                                        :media="(int) $sport->images[0]" />
                                    <div class="order-2 mt-2 md:order-1">
                                        <a href=""
                                            class="text-xl font-semibold font-head-ebgaramond hover:text-indigo-600">
                                            {{ $sport->name }}
                                        </a>
                ...
                <div class="order-1 lg:col-span-4 md:col-span-12 lg:order-2">
                    <x-curator-glider class="mx-auto lg:w-auto md:w-80" :media="$settings->welcomeImage" />
                </div>
                        ...
                        @foreach ($sports as $sport)
                            @if ($loop->even)
                                <div class="duration-500 group">
                                    ...
                                    <div class="mt-2">
                                        <a href=""
                                            class="text-xl font-semibold font-head-ebgaramond hover:text-indigo-600">The
                                            {{ $sport->name }}
                                        </a>
                                        <p class="mt-3 text-slate-400">{{ $sport->short_description }}</p>
                                    </div>
    ...
    <div class="container relative mt-16 md:mt-24">
        <div class="grid md:grid-cols-2 gap-[30px] items-center">
            <div class="">
                <img src="{{ $settings->sportSectionImage }}" class="rounded shadow dark:shadow-gray-800"
                    alt="">
            </div>
            ...
            <div class="lg:ms-8">
                <h4
                    class="mb-4 text-3xl font-medium leading-normal md:text-4xl md:leading-normal font-head-ebgaramond">
                    {{ $settings->sportSectionTitle }}</h4>
                <p class="max-w-xl mx-auto text-slate-400">{{ $settings->sportSectionMessage }}</p>

                <div class="grid grid-cols-1 gap-[30px] mt-6">
                    <div class="flex items-center duration-500 group">
                        <x-curator-glider class="w-16 h-16 p-3 bg-white rounded-full shadow-md dark:shadow-gray-800"
                            :media="$settings->sportSectionImage1" />
                        <div class="flex-1 ms-3">
                            <a href=""
                                class="text-xl font-semibold font-head-ebgaramond hover:text-indigo-600">{{ $settings->sportSectionTitle1 }}</a>
                            <p class="mt-3 text-slate-400">{{ $settings->sportSectionMessage1 }}</p>
                        </div>
                    </div>

                    <div class="flex items-center duration-500 group">
                        <x-curator-glider class="w-16 h-16 p-3 bg-white rounded-full shadow-md dark:shadow-gray-800"
                            :media="$settings->sportSectionImage2" />
                        <div class="flex-1 ms-3">
                            <a href=""
                                class="text-xl font-semibold font-head-ebgaramond hover:text-indigo-600">{{ $settings->sportSectionTitle2 }}</a>
                            <p class="mt-3 text-slate-400">{{ $settings->sportSectionMessage2 }}</p>
                        </div>
                    </div>
                </div>
